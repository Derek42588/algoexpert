...
export function longestPeak(array: number[]): number {
  ...
  for (let i = 1; i <= array.length - 2; i++) {
    let isPeak: boolean = array[i] > array[i + 1] && array[i] > array[i - 1];

    if (isPeak) {
      debugger;
      let peakLength = expandOut(array, i - 1, i + 1);
      if (peakLength > longest) longest = peakLength;
    }
  }
...
function expandOut(array: number[], left: number, right: number): number {
  let length = 3;
